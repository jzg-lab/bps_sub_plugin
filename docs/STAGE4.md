...
- [x] **I6 测试环境实测**：版本 0.4.0，打包上传。测：
  ...
  5. 宿主日志无错误，统计合理。
- [x] **I7 收尾**：更新 README、PLAN.md、本文；提交并推送。
...
- **2026-09-25 I6 △**（插件 0.4.0，测试环境）：
  - `image_support` 开关、16 字段配置、图片统计都正常上线；`tool_relay=false` 回归验证过(阶段3已证)。
  - **附件上传对真实 basispoints 返回 200 + file_id**（直接用账号 1 令牌探针，两次都成功）。
  - **用 sub2api 0.2.8 真实插件运行时**跑了图片请求(临时测试,跑完删):请求正确进入插件、执行到
    上传步骤(尝试连 bps 附件端点)、干净返回传输错误、不崩溃。图片路径在真实宿主运行时里走通。
  - **未能拿到端到端 200 视觉回复**:两个测试账号在 basispoints 上都被额度/付费限制
    (账号1返回402,账号2 error 状态),宿主账号调度直接 `account_select_failed`,请求到不了上游。
    这是**账号状态**,不是插件问题——上传机制(200)、body 改写、路由都已分别验证。
    换一个额度正常的账号即可拿到真实视觉回复。
- **2026-09-25 I7 ✅**：README、PLAN.md 更新到阶段 4 完成;0.4.0 部署测试环境;提交并推送。
